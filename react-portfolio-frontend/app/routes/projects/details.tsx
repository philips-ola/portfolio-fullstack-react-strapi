...
export async function loader({request, params}:Route.LoaderArgs){
    const{id} = params;
    const res = await fetch(`${import.meta.env.VITE_STRAPI_URL}/api/projects?filters[documentId][$eq]=${id}&populate=*`);
...
const ProductDetailPage = ({loaderData}:Route.ComponentProps) => {
    ...
    return(
        <>
           <Link to='/projects' className="flex item-center text-blue-400 hover:text-blue-500 mb-6 transition">
